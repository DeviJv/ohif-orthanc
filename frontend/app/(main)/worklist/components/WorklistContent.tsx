...
import { getColumns, WorklistTableMeta } from "./columns";
import { WorklistToolbar } from "./worklist-toolbar";
import { StudyDetailRow } from "./study-detail-row";
import { DeleteStudyDialog } from "./delete-study-dialog";
import { BulkDeleteStudyDialog } from "./bulk-delete-dialog";
...
export default function WorklistContent() {
    ...
    // We can still use isMounted to prevent any early render before client is ready
    ...
    useEffect(() => {
        setIsMounted(true);
        setSelectedStudyUID(searchParams.get("viewer"));
        setViewerMode(normalizeViewerMode(searchParams.get("mode")));
    }, [searchParams]);

    const viewerStudyUID = selectedStudyUID;
    const showViewer = isMounted && Boolean(viewerStudyUID);
                        ...
                        <div className="absolute top-0 left-0 p-0 z-20 pointer-events-none">
                            <div className="flex items-center gap-2 bg-[#090c29] border-r border-b border-slate-700/50 rounded-br-xl p-1.5 pr-5 shadow-2xl pointer-events-auto">
                                ...
                                <div className="flex flex-col justify-center">
                                    ...
                                    <span className="text-[10px] text-slate-400 font-medium leading-tight">Viewer v1.0</span>
                                </div>
    ...
    return (
        <div className="p-6 w-full space-y-6">
                            ...
                            Array.from({ length: 10 }).map((_, i) => (
                                <TableRow key={i} className="hover:bg-transparent border-b">
                                    <TableCell className="py-4">
                                        <div className="flex items-center justify-center pr-2"><Skeleton className="size-4 rounded" /></div>
                                    </TableCell>
                                    <TableCell className="py-4"><Skeleton className="size-8 rounded-md" /></TableCell>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className="shadow-sm">Previous</Button>
                    <div className="flex items-center gap-1 text-sm font-medium px-2">Page {table.getState().pagination.pageIndex + 1} of {table.getPageCount()}</div>
                    <Button variant="outline" size="sm" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className="shadow-sm">Next</Button>
                </div>
            </div>
