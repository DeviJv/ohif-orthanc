...
export function WebhookMonitor() {
    ...
    return (
        <div className="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden transition-all duration-300 backdrop-blur-sm">
            <div 
            ...
            >
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-lg">
                        <HugeiconsIcon icon={Clock01Icon} className="size-5" />
                    </div>
                    <div>
                        <h3 className="font-bold text-slate-900 dark:text-slate-100">Monitor Transmisi Gambar</h3>
            ...
            {isOpen && (
                <div className="p-4 pt-0 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex justify-between items-center mb-4 border-t border-slate-200 dark:border-slate-800 pt-4">
                    ...
                    >
                        {loading ? (
                            ...
                            <div className="py-12 text-center text-slate-400 dark:text-slate-600 italic text-sm">
                            ...
                            <>
                                {logs.map((log) => (
                                    <div 
                                        key={log.id} 
                                        className="p-3 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm hover:shadow-md transition-shadow flex items-start gap-4"
                                    >
                                        <div className={cn(
                                            "p-2 rounded-full shrink-0",
                                            log.status === "SUCCESS" ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400" : "bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400"
                                        )}>
                                            <HugeiconsIcon 
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start gap-2">
                                                ...
                                                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium whitespace-nowrap bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">
                                            ...
                                            <div className="mt-2 flex items-center gap-2 overflow-x-auto hide-scrollbar">
                                                <code className="text-[10px] bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-500 dark:text-slate-400 font-mono">
